...
const EDITIONS = {
  classic: { title: "Classic Personal Discovery", description: "A balanced personal report for discovery, reflection and birthday context." },
...
function reportHtml(report) {
  ...
  return `
    <article class="report-preview edition-${escapeHtml(report.edition || selectedEdition)}">
      ...
      <div class="report-grid">
        <p><strong>Key traits:</strong> ${escapeHtml(report.keyTraits)}</p><p><strong>Strengths:</strong> ${escapeHtml(report.strengths)}</p>
        <p><strong>Challenges:</strong> ${escapeHtml(report.challenges)}</p><p><strong>Communication:</strong> ${escapeHtml(report.communicationStyle)}</p>
      </div>
      <h3>Life areas</h3><p><strong>Relationships:</strong> ${escapeHtml(report.relationship)}</p>
      <p><strong>Learning:</strong> ${escapeHtml(report.learning)}</p><p><strong>Work and goals:</strong> ${escapeHtml(report.workCareer)} ${escapeHtml(report.goals)}</p>
...
function updateEditionForm() {
  ...
  document.querySelector("#coupleSecondName").required = selectedEdition === "couples";
  document.querySelector("#coupleSecondBirthDate").required = selectedEdition === "couples";
}
...
reportForm.addEventListener("submit", async event => {
  ...
  latestReportInput = {
    ...
    secondName: document.querySelector("#coupleSecondName").value.trim(),
    secondBirthDate: document.querySelector("#coupleSecondBirthDate").value,
...
compareForm.addEventListener("submit", async event => {
  event.preventDefault();
  const payload = {
    name: document.querySelector("#firstName").value.trim(), birthDate: document.querySelector("#firstDate").value,
    secondName: document.querySelector("#secondName").value.trim(), secondBirthDate: document.querySelector("#secondDate").value
  };
